...
def as_editor_payload(rows: list[BudgetEditorRow]) -> dict[str, list[dict]]:
    ...
    enough history to compute either), and month_actual - the selected
    (year, month)'s own actual so far, always a real figure, never unset
    (Issue #135) - see dashboard.queries.get_budget_editor.
    """
