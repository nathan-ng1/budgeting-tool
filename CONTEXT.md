...
**Installation Pack**:
The `setup.bat` / `update.bat` / `docs/setup-guide.html` bundle that takes a new user from nothing installed to a running Dashboard (and, if they hold an AI subscription, the Statement Export pipeline too). `setup.bat` is a standalone bootstrapper — runnable before Git or a clone exist — distributed as a GitHub Release asset; it branches on whether the user holds an AI subscription, since automated categorisation has no manual/null backend and so cannot be made to work without one. See [ADR-0017](./docs/adr/0017-installation-pack-is-a-bootstrapper-distributed-via-releases.md), [ADR-0018](./docs/adr/0018-non-ai-path-is-dashboard-only.md), [ADR-0019](./docs/adr/0019-update-checks-are-automatic-updates-are-manual.md).
_Avoid_: Installer, setup script (for the pack as a whole — `setup.bat` is fine for the script itself)
