...
def test_candidates_with_no_matching_existing_rows_are_all_written(make_candidate):
    candidates = [
        make_candidate(notes="Woolworths"),
        make_candidate(notes="Coles"),
    ]

    result = resolve_writes(candidates, existing_rows=[])
...
def test_candidate_matching_existing_row_on_full_date_amount_and_notes_is_skipped(
    make_candidate, make_logged
):
    candidate = make_candidate(notes="Woolworths")
    existing = make_logged(notes="Woolworths")

    result = resolve_writes([candidate], existing_rows=[existing])

    assert result.to_write == []
    assert result.skipped == [candidate]


def test_near_miss_on_amount_or_date_is_written_not_skipped(make_candidate, make_logged):
    ...
    result = resolve_writes([different_amount, different_date], existing_rows=[existing])

    assert result.to_write == [different_amount, different_date]
    assert result.skipped == []


def test_due_recurring_occurrence_already_in_log_is_merged_in_and_then_skipped(make_logged):
    ...
    existing = make_logged(date=date(2026, 8, 5), amount=5000.0, notes="Employer Pty Ltd")
...
def test_combined_candidates_and_recurring_occurrences_dedupe_as_one_unit(
    ...
    existing_rows = [
        make_logged(date=date(2026, 8, 4), amount=15.00, notes="Coles"),
        make_logged(date=date(2026, 8, 1), amount=500.0, notes="Tenant Payment"),
    ]
