...
def test_candidates_with_no_matching_existing_rows_are_all_written(
    ...
    result = resolve_writes(candidates, existing_rows=store.read_existing_rows())
...
def test_candidate_matching_existing_row_on_full_date_amount_and_notes_is_skipped(
    make_candidate, make_existing_row, fake_store
):
    candidate = make_candidate(notes="Woolworths")
    store = fake_store(existing_rows=[make_existing_row(notes="Woolworths")])

    result = resolve_writes([candidate], existing_rows=store.read_existing_rows())
...
def test_candidate_still_carrying_the_source_transactions_negative_sign_dedupes_against_the_positive_logged_amount(
    ...
    # writer normalises to positive only when it writes (see database.store),
...
def test_near_miss_on_amount_or_date_is_written_not_skipped(
    ...
    result = resolve_writes(
        [different_amount, different_date], existing_rows=store.read_existing_rows()
    )
