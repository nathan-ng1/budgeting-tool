"""Where Advisor configuration comes from - mirrors
tests/categorisation/test_config.py exactly, since advisor.config is a
behavioral copy of categorisation.config (Issue #65).
...
def test_reads_settings_from_the_repo_root_env_file(tmp_path: Path, monkeypatch):
    monkeypatch.setattr(config, "REPO_ROOT", write_env(tmp_path, "ADVISOR_BACKEND=codex\n"))

    assert config.load()["ADVISOR_BACKEND"] == "codex"


def test_a_real_environment_variable_wins_over_the_env_file(tmp_path: Path, monkeypatch):
    monkeypatch.setattr(config, "REPO_ROOT", write_env(tmp_path, "ADVISOR_BACKEND=codex\n"))
    monkeypatch.setenv("ADVISOR_BACKEND", "claude")

    assert config.load()["ADVISOR_BACKEND"] == "claude"
...
def test_a_missing_env_file_is_not_an_error(tmp_path: Path, monkeypatch):
    ...
    assert "ADVISOR_BACKEND" not in config.load()
